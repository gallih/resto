...
class Apesanan_min extends CI_Controller {
	...
	{		
		...
		$config['base_url'] =base_url().'apesanan_min/index';
		$config['total_rows'] = $jumlah;
		$config['per_page']=3;

		$this->pagination->initialize($config);
		$data['halaman'] = $this->pagination->create_links();
		$data['datadb']= $this->model_apesanan->lihat($config['per_page'],$id,'q_pesan','Minuman');
		...
		$this->load->view('admin/pesanan/view_pesanan_min',$data);
	}
	...
	function cetakminum()
    {
        ...
        $pdf->SetFont('Arial','',9);
        $pdf->Cell(4,0.5,"Pesanan Minuman yang Belum",0,0,'C');
        $pdf->Ln(0.5);
        $pdf->Cell(5,0,"-----------------",0,0,'C');
        $pdf->Ln(0.5);
        	foreach ($data->result() as $mik ) { 
        		$pdf->Cell(4,0.5,$mik->item,0,0,'L');
        		$pdf->Cell(1,0.5,$mik->Sum_jml,0,0,'R');
        	}
        // $pdf->AutoPrint(true);
        $pdf->AddPage();
        $pdf->Output();
